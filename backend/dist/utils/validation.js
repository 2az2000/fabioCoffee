...
exports.createItemSchema = zod_1.z.object({
    name: zod_1.z.string().min(1, 'Item name is required').max(100, 'Item name must be less than 100 characters'),
    description: zod_1.z.string().nullable().optional(),
    price: zod_1.z.number().positive('Price must be positive').max(10000, 'Price must be less than 10000'),
    imageUrl: zod_1.z.string().url('Must be a valid URL').nullable().optional().or(zod_1.z.literal('')),
    isActive: zod_1.z.boolean().optional().default(true),
...
exports.createOrderSchema = zod_1.z.object({
    ...
    items: zod_1.z.array(zod_1.z.object({
        ...
        quantity: zod_1.z.number().int().positive('Quantity must be positive').max(100, 'Quantity must be 100 or less'),
