...
import { Coffee, Package, Edit2, Trash2, Save, X, Plus, AlertTriangle, Filter } from "lucide-react";
...
export default function ItemsPage() {
  ...
  const [editForm, setEditForm] = useState({ name: '', description: '', price: '', categoryId: '', imageUrl: '', isActive: true });
  const [isSaving, setIsSaving] = useState(false);
  const [deletingId, setDeletingId] = useState<string | null>(null);
  const [showDeleteModal, setShowDeleteModal] = useState<string | null>(null);
  const [showAddModal, setShowAddModal] = useState(false);
  const [addForm, setAddForm] = useState({ name: '', description: '', price: '', categoryId: '', imageUrl: '', isActive: true });
  ...
  const handleCloseModal = () => {
    setEditingItem(null);
    setEditForm({ name: '', description: '', price: '', categoryId: '', imageUrl: '', isActive: true });
  };
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        setAddForm({ name: '', description: '', price: '', categoryId: '', imageUrl: '', isActive: true });
      }
    } finally {
      setIsSaving(false);
    }
    ...
    <div className="space-y-8">
        ...
        <div className="flex items-center gap-3">
          {!isLoading && (
            <div className="text-sm bg-gray-100 px-4 py-2 rounded-lg text-gray-700">
              تعداد آیتم: <span className="font-semibold">{filteredItems.length}</span>
            </div>
          )}
          <button
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-lg font-semibold text-gray-900">فیلتر بر اساس دسته‌بندی</h3>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          ...
          >
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 border-b border-gray-100">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-bold text-gray-900">ویرایش آیتم</h3>
                  <p className="text-sm text-gray-600 mt-1">تغییر اطلاعات آیتم</p>
                </div>
                <button
            ...
            <div className="flex items-center justify-end gap-3 p-6 border-t border-gray-100">
              ...
              <button
                onClick={handleSave}
                disabled={isSaving || !editForm.name.trim() || !editForm.price || !editForm.categoryId}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          ...
          >
            ...
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-6 border-b border-gray-100">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-bold text-gray-900">افزودن آیتم جدید</h3>
                  <p className="text-sm text-gray-600 mt-1">ایجاد آیتم جدید</p>
                </div>
                <button
                  onClick={() => {
                    setShowAddModal(false);
            ...
            <div className="flex items-center justify-end gap-3 p-6 border-t border-gray-100">
              ...
              <button
                onClick={handleAdd}
                disabled={isSaving || !addForm.name.trim() || !addForm.price || !addForm.categoryId}
